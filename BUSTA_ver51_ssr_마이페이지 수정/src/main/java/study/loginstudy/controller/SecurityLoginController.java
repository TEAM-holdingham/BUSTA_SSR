...
//
    ...
    public String showDeleteAccountPage() {
        return "delete_account"; // 탈퇴 확인 페이지로 이동 (delete_account.html 필요)
    }
    ...
    public String deleteAccount(Authentication authentication) {
        if (authentication != null) {
            String loginId = authentication.getName();
            userService.deleteUserByLoginId(loginId); // 사용자 삭제 로직
            return "redirect:/"; // 로그아웃 후 홈으로 리다이렉트
        }
        return "redirect:/security-login/login"; // 인증되지 않은 사용자는 로그인 페이지로 리다이렉트
    }
//
